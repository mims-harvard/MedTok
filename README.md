...
MedTok serves the Transformer-based EHR foundation models as a tokenizer. MedTok tokenizes each medical code into tokens and then these tokens are adopted by Transformer-based EHR foundation models as input to finish its training or pretraining and thus the inference. Here, we also show an example how to integrate our MedTok with a simple transformer-based model on MIMIC III, MIMIC IV, and EHRShot datasets.
